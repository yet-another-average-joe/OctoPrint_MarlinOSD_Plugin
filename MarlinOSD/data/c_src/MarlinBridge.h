...
public:
	...
	static unsigned char spiBuffer[MARLIN_BMP_SIZE]; // bitmap, 1 bit per pixel, big endian
	static bool spiError;

	static void open();			// opens SPI1 ; is fails, switches to demo mode
	static bool read();			// reads one frame to spiBuffer (Marlin UI bitmap, MARLIN_BMP_SIZE bytes )
